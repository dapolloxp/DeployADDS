...
Param(
    ...
    [string] $ResourceGroupName = 'DCTestDeployment',
	#[string] [Parameter(Mandatory=$true)] $ScriptsResourceGroup,
	[string] $ScriptsResourceGroup = 'StorageTemplates',
	#[string] [Parameter(Mandatory=$true)] $ScriptsStorageAccountName,
	[string] $ScriptsStorageAccountName = 'apolinarsascripts',
...
$SC2 = (Get-AzureRmStorageAccount -ResourceGroupName $ScriptsResourceGroup -Name $ScriptsStorageAccountName).Context 

# Writing out SC
Write-Output $SC2
$SASToken = New-AzureStorageContainerSASToken -container scripts -context $SC2 -Permission r -ExpiryTime (Get-Date).AddHours(4)

# Writing out SASToken
